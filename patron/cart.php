...
<head>
    ...
    <style>
        ...
        /* Custom styles to remove background and border of the button */
        .btn-custom {
            background-color: transparent;
            border: none;
            padding: 0; /* Remove padding to make the button content align properly */
        }
                                                        ...
                                                        <div class="card mb-3">
                                                            <div class="card-header" style="background-color: #EADBC8; font-size: 20px; font-weight: bold;">
                                                                ...
                                                                <select class="form-control" id="bk_cat" name="bk_cat" required onchange="redirectToPage()" style="width: 20%;">
                                                                    <option value="cart.php" selected>My Basket</option>
                                                                    <option value="pending.php">Pending Request(s)</option>
                                                                    <option value="approved.php">Approved</option>
                                                            ...
                                                            <div class="card-body">
                                                                ...
                                                                <table id="dataTable" class="table table-bordered table-striped" style="font-size:15px">
                                                                    ...
                                                                    <tbody>
                                                                        <?php
                                                                            $query = mysqli_query($conn, "SELECT cart.*, book.bk_title, book.author, book.edition, book.copies, transaction.status 
                                                                                                                FROM cart 
                                                                                                                ...
                                                                                                                WHERE cart.user_id = '" . $_SESSION['user_id'] . "' 
                                                                                                                    AND (transaction.status IS NULL OR transaction.status NOT IN ('disapproved', 'canceled', 'pending', 'approved', 'returned', 'borrowed'))
                                                                        ...
                                                                        <?php
                                                                        include 'viewDetailModal.php';
                                                                    ?>
                                                                            <tr>
                                                                            <td id="modalButton" style="cursor: pointer;">
                                                                                <a href="../includes/inc_patmodal/edit_patmodal.php?edit=<?php echo $row['ISBN']?>" class="btn btn-custom" style="font-size:15px" data-bs-toggle="modal" data-bs-target="#viewDetail">
                                                                                    <?php echo $row['bk_title'];?>
                                                                                </a>
                                                                            </td>
                                                                                <td class="text-center">
                                                                                <?php if ($row['copies'] == 0): ?>
                                                                                    <button class="btn btn-secondary me-md-3 mb-3 mb-md-0" disabled title="Not Available">Not Available</button>
                                                                                <?php else: ?>
                                                                                    <a title="Request" href="../includes/filesproccess/circulationProcess.php?requestbook=<?php echo $row['ISBN'] ?>" class="btn btn-success btn-sm">
